(() => {
  ...
  async function loadPublicData(){
    ...
    try{
      ...
      if(settings[0]){const s=settings[0];restaurant={...restaurant,name:s.name,phoneDisplay:s.phone,phoneHref:String(s.phone||'').replace(/[^+\d]/g,''),address:s.address,city:s.city_postcode,email:s.email,tagline:s.tagline,description:s.description,rating:s.rating};heroTitle.textContent=s.hero_title||s.name;heroText.textContent=s.hero_text||s.tagline;specialEyebrow.textContent=s.daily_label||'';specialName.textContent=s.daily_title||'';specialDescription.textContent=s.daily_description||'';specialPrice.textContent=money(s.daily_price||0);if(s.daily_image_url){const position=`${s.daily_image_position_x??50}% ${s.daily_image_position_y??50}%`;specialArt.style.overflow='hidden';specialArt.style.backgroundImage='none';const img=document.createElement('img');img.src=s.daily_image_url;img.alt='Photo du plat du moment';img.style.cssText='width:100%;height:100%;object-fit:contain;display:block';img.style.objectPosition=position;img.style.transformOrigin=position;img.style.transform=`scale(${Math.min(250,Math.max(25,Number(s.daily_image_zoom??100)))/100})`;specialArt.replaceChildren(img);}}
