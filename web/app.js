(() => {
  ...
  async function rest(path,options={}){const response=await fetch(`${cfg.supabase.url}/rest/v1/${path}`,{...options,headers:{apikey:cfg.supabase.publishableKey,Authorization:`Bearer ${cfg.supabase.publishableKey}`,'Content-Type':'application/json',...(options.headers||{})}});const body=await response.text();if(!response.ok)throw new Error(body||`HTTP ${response.status}`);if(!body)return null;try{return JSON.parse(body)}catch{return body}}
