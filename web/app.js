(() => {
  ...
  async function loadPublicData(){
    ...
    try{
      ...
      if(settings[0]){const s=settings[0];restaurant={...restaurant,name:s.name,phoneDisplay:s.phone,phoneHref:String(s.phone||'').replace(/[^+\d]/g,''),address:s.address,city:s.city_postcode,email:s.email,tagline:s.tagline,description:s.description,rating:s.rating};}
      if(items.length){menu=items.map(item=>({category:item.category,name:item.name,description:item.description,price:Number(item.price),vegetarian:item.is_vegetarian}));const daily=items.find(item=>item.is_daily_special);if(daily){specialName.textContent=daily.name;specialDescription.textContent=daily.description;specialPrice.textContent=money(daily.price);}}
  ...
  async function init(){
    ...
    const special=cfg.dailySpecial||{};specialEyebrow.textContent=special.eyebrow||'';specialName.textContent=special.name||'';specialDescription.textContent=special.description||'';specialPrice.textContent=money(special.price||0);
