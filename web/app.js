(() => {
  ...
  async function rest(path,options={}){const response=await fetch(`${cfg.supabase.url}/rest/v1/${path}`,{...options,headers:{apikey:cfg.supabase.publishableKey,Authorization:`Bearer ${cfg.supabase.publishableKey}`,'Content-Type':'application/json',...(options.headers||{})}});if(!response.ok)throw new Error(await response.text());if(response.status===204)return null;return response.json();}
